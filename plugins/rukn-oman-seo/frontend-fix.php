...
final class Rukn_Oman_Frontend
{
    ...
    public static function head_css()
    {
        echo '<style id="rukn-oman-fix-css">'
            ...
            . 'nav.menu{display:flex;flex-wrap:wrap;gap:8px 18px;align-items:center}'
            . 'nav.menu a{color:inherit;font-weight:700;font-size:14.5px;text-decoration:none}'
            . 'nav.menu a:hover{color:var(--turq,#1FB5A3)}'
            . '#ruknMob .rukn-mob-links{display:flex;flex-direction:column;gap:10px;margin:18px 0}'
            . '#ruknMob .rukn-mob-links a{font-weight:800;color:#0A1F4E;text-decoration:none;padding:10px 4px;border-bottom:1px solid #eef2f6}'
            . '</style>' . "\n";
    ...
    public static function menu_items()
    {
        ...
        return [
            ...
            ['واتساب', self::WA_HREF],
            ['خريطة الموقع', home_url('/html-sitemap/')],
        ];
    }

    public static function nav_html()
    {
        $out = '';
        foreach (self::menu_items() as [$label, $url]) {
    ...
    public static function rewrite($html)
    {
        ...
        $html = preg_replace('/<nav class="menu"><\/nav>/', '<nav class="menu">' . self::nav_html() . '</nav>', $html);

        $mob = '<div class="rukn-mob-links">' . self::nav_html() . '</div>';
